...
function SpaceView({ spaceId, itemId }: { spaceId: string; itemId: string }) {
  ...
  const form = useForm({
    initialValues: {
      title: proposalQuery.data?.proposal.title || "",
      body: proposalQuery.data?.proposal.body || "",
    },
    validate: {
      title: (value) =>
  ...
  const mutation = api.proposal.updateProposal.useMutation({
    onSuccess(data) {
      void utils.space.getSpace.invalidate();
      void router.push(`/app/space/${spaceId}/proposal/${data.id}`);
    },
  });
  ...
  if (!proposalQuery.data) return <div>Did not find calendar event.</div>;
