...
function SpaceView({ spaceId, itemId }: { spaceId: string; itemId: string }) {
  ...
  const feedbackQuery = api.feedback.getFeedbackRound.useQuery({
    itemId,
  });
  ...
  const form = useForm({
    initialValues: {
      title: feedbackQuery.data?.title || "",
      body: feedbackQuery.data?.body || "",
    },
    validate: {
      title: (value) =>
  ...
  const mutation = api.feedback.updateFeedbackRound.useMutation({
    onSuccess(data) {
      void utils.space.getSpace.invalidate();
      void router.push(`/app/space/${spaceId}/feedback/${data.id}`);
    },
  });
  ...
  useEffect(() => {
    if (feedbackQuery.data) {
      form.setValues(feedbackQuery.data);
      if(editor) {
        editor.commands.setContent(feedbackQuery.data.body);
      }
    }
   }, [editor, feedbackQuery.data])
  if (spaceQuery.isPlaceholderData) return <div>Loading ...</div>;
  if (!spaceQuery.data) return <div>Did not find space.</div>;
  if (feedbackQuery.isLoading) return <div>Loading ...</div>;
  if (!feedbackQuery.data) return <div>Did not find calendar event.</div>;
